        ...
        fetch(`/weather?location=${location}`).then((res) => {
            res.json().then((data) => {
                if(data.error) {
                    ...
                    messageOne.textContent = `Today in ${data.dailySummary.location} it's ${data.dailySummary.summary} and the temperature is ${data.dailySummary.temp} C and ${Math.round(Number(data.dailySummary.constProb)*100)} % chance of rain.`
                }
            })   
        }) 
        } else {
           fetch(`/weather?lat=${location.coords.latitude}&long=${location.coords.longitude}`).then((res) => {
                res.json().then((data) => {
                    if(data.error) {
                        messageOne.textContent = data.error
                    } else {
